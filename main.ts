...
function More_Mazes (Even_More_Mazes: number) {
    ...
    while (Youvebeenhere.length > 0) {
        ...
        MaybeHere = []
        WhereYouAre = cursor.tilemapLocation()
        ...
        if (WhereYouAre.column > 0 && cursor.tileKindAt(TileDirection.Left, assets.tile`transparency16`)) {
            MaybeHere.push(tiles.getTileLocation(WhereYouAre.column - 1, WhereYouAre.row))
        }
        if (WhereYouAre.row > 0 && cursor.tileKindAt(TileDirection.Top, assets.tile`transparency16`)) {
            MaybeHere.push(tiles.getTileLocation(WhereYouAre.column, WhereYouAre.row - 1))
        }
        if (WhereYouAre.row < last_row && cursor.tileKindAt(TileDirection.Bottom, assets.tile`transparency16`)) {
            MaybeHere.push(tiles.getTileLocation(WhereYouAre.column, WhereYouAre.row + 1))
        }
        tentacle = cursor.tilemapLocation()
        while (MaybeHere.length > 0) {
            tiles.placeOnTile(cursor, MaybeHere.removeAt(randint(0, MaybeHere.length - 1)))
            Count = 0
    ...
    if (Even_More_Mazes == 2) {
        for (let Value of MazeFloor) {
            tiles.setTileAt(Value, assets.tile`myTile0`)
        }
    }
    for (let Value22 of Walls) {
        if (Even_More_Mazes == 2) {
            tiles.setTileAt(Value22, assets.tile`myTile3`)
        } else if (Even_More_Mazes == 1) {
            tiles.setTileAt(Value22, assets.tile`myTile`)
        }
        tiles.setWallAt(Value22, true)
    }
}
sprites.onOverlap(SpriteKind.KittyCat, SpriteKind.Door3, function (sprite, otherSprite) {
...
mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.One)).setVelocity(100, 100)
